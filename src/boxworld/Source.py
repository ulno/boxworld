...
class Source(object):
	...
	def guenther(self, light, temperature):
		...
		The Guenther terpene emission algorithm	
	 	...
	 	temperature. 
		'''
		return self.e0 * self.CL(light) * self.CT(temperature)
	...
	def getEmission(self, light, temperature, fnair, time_delta):
		'''
		Accessor method to read out the emission from that source.
		'''
		# print "fnair ", fnair
		return self.guenther(light, temperature) * fnair * 1e-6 * time_delta # 1e-6 = dilution factor into volume!
